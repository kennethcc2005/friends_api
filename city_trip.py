...
def get_city_trip_data(state, city, n_days, full_day=True, regular=True, visit_speed='normal', visible=True):
    ...
    if not helpers.check_full_trip_id(full_trip_id):
        ...
        available_city_ndays = len(city_poi_list_info)/num_poi_per_day
        if available_city_ndays < n_days:
            city_poi_list_info = helpers.db_city_and_surrounding_poi(city, state, n_days, num_poi_per_day)
