...
def serach_city_state(city_state):
    ...
    if ',' in city_state:
        ...
        cur.execute("SELECT city, state_abb, concat_ws(', ',city::text, state_abb::text) FROM poi_detail_table WHERE city % '{0}' AND (state % '{1}' or state_abb % '{1}') order by similarity(city, '{0}') desc limit 5;".format(city, state))
    else:
        city = city_state.strip()
        cur.execute("SELECT city, state_abb, concat_ws(', ',city::text, state_abb::text) FROM poi_detail_table WHERE city % '{0}' order by similarity(city, '{0}') desc limit 5;".format(city))
    c = cur.fetchall()
    conn.close()
    return c